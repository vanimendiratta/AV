...
const BusinessAnalytics = ({ dataId }) => {
  ...
  const handleDownload = () => {
    const fileUrl = '/QPR.pdf'; // Path relative to the public folder
    const a = document.createElement('a');
    a.href = fileUrl;
    a.download = 'qpr.pdf'; // The name of the downloaded file
  ...
  return (
    <div className="flex flex-col lg:flex-row justify-between px-6 py-12 bg-[#F5F5F5] text-[#333]">
      ...
      <div className="flex-1 mb-8 lg:mb-0 lg:mr-8 p-6 bg-[#E0D9C4] rounded-lg shadow-lg">
        ...
        <p className="mb-4 text-[#1E2A1F]">Eligibility-(pursuing or completed- any specialisation): {data.eligibility}</p>
